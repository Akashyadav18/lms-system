...
import CategoryItem from '../browse/_components/CategoryItem'
...
const Dashboard = () => {
  ...
  const getUserCourse = async () => {
    await GetUserCourseList(user.primaryEmailAddress.emailAddress)
      .then(res => {
        console.log("Dashboard :", res?.userEnrollCourses);
        if (res) {
  ...
  return (
    <div>
      ...
      {userCourseList ?
        ...
        :
        <div className='flex justify-center items-center text-[20px] mt-20 text-gray-500'>You don't have any course enrolled!</div>
      }
    </div>
  )
}
